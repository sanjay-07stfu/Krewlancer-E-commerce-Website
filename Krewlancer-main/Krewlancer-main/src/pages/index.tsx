  ...
  return (
    <div id={sectionId} className="flex flex-col justify-center px-6 md:px-24 scroll-mt-52">
      <div className="max-w-400 w-full mx-auto">
        ...
        <div className="mt-8">
          <Link to="/view-all" className="text-xs uppercase tracking-widest border-b border-white/20 pb-1 hover:border-white transition-all text-gray-400 hover:text-white">
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const runHero = () => {
      ctxHost = gsap.context(() => {
        gsap.set(".hero-shop-now", { autoAlpha: 0, y: 16 })
        gsap.to(".hero-shop-now", {
          autoAlpha: 1,
          y: 0,
          duration: 0.5,
          ease: "power2.out",
  ...
  const getSlideShopLink = (slide: any) => {
    const params = new URLSearchParams()
    const category = String(slide?.target_category || "").trim()
    const subcategory = String(slide?.target_subcategory || "").trim()
  ...
  return (
    <>
      ...
      <div ref={rootRef} className="bg-[#030303] text-[#e8e8e3] overflow-x-hidden min-h-screen">
        ...
        {/* HERO CAROUSEL */}
        <section className="hero-carousel relative h-screen bg-[#030303] md:mt-20 md:h-[calc(100vh-5rem)]">
            ...
            <div className="embla__container flex h-full">
              {slides.map((slide: any, idx: number) => (
                <div key={idx} className="embla__slide relative flex-[0_0_100%] h-full w-full">
                  ...
                  <div className="hero-shop-now absolute bottom-10 right-6 z-30 flex justify-end px-0 md:bottom-12 md:right-10">
                    <Link
                      to={getSlideShopLink(slide)}
                      className="inline-flex min-w-35 max-w-full items-center justify-center rounded-full border border-[#b8a471]/45 bg-black/35 px-6 py-3 text-center font-serif text-[11px] uppercase tracking-[0.24em] text-[#cfbc86] backdrop-blur-sm transition-all duration-500 hover:border-[#b8a471] hover:bg-[#b8a471]/20 hover:text-[#ead8a4] md:min-w-44 md:px-8 md:py-3.5 md:text-[12px] active:scale-95"
                    >
                      Shop Now
                    </Link>
                  </div>
