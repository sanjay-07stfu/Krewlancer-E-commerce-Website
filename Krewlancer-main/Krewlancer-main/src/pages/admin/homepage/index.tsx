...
function HeroSlideEditor({
    ...
    const targetCategory = slide.target_category || ""
    const selectedCategory = categories.find((c) => c.name === targetCategory)
    const availableSubcategories = Array.isArray(selectedCategory?.subcategories)
        ? selectedCategory.subcategories.map((sub: unknown) => String(sub || "").trim()).filter(Boolean)
        : []
                ...
                <div className="flex-1 space-y-6">
                    <div className="space-y-6">
                                                ...
                                                <div
                                                    ...
                                                    onClick={() => {
                                                        onUpdate(index, {
                                                            ...
                                                            target_category: String(p.category || "").trim(),
                                                            target_subcategory: String(p.subcategory || "").trim()
                                                        })
                                                        setIsOpen(false)
                                                    }}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <Label className="text-[9px] uppercase tracking-[0.3em] text-gray-500">Shop Now Category</Label>
                                <Select
                                    value={targetCategory || "none"}
                                    onValueChange={(value) => {
                                        const nextCategory = value === "none" ? "" : value
                                        const shouldKeepSubcategory = nextCategory === targetCategory
                                        onUpdate(index, {
                                            target_category: nextCategory,
                                            target_subcategory: shouldKeepSubcategory ? (slide.target_subcategory || "") : ""
                                        })
                                    }}
                                >
                                    <SelectTrigger className="bg-transparent border-white/10 rounded-none text-[10px] uppercase tracking-widest h-10">
                                        <SelectValue placeholder="Select category" />
                                    </SelectTrigger>
                                    <SelectContent className="bg-[#030303] border-white/10 text-[#e8e8e3]">
                                        <SelectItem value="none">NONE (OPEN VIEW ALL)</SelectItem>
                                        {categories.map((c) => (
                                            <SelectItem key={c.id || c.name} value={String(c.name)}>
                            ...
                            <div className="space-y-2">
                                <Label className="text-[9px] uppercase tracking-[0.3em] text-gray-500">Shop Now Subcategory</Label>
                                <Select
                                    value={slide.target_subcategory || "all"}
                                    onValueChange={(value) => onUpdate(index, { target_subcategory: value === "all" ? "" : value })}
                                    disabled={!targetCategory || availableSubcategories.length === 0}
                                >
                                    <SelectTrigger className="bg-transparent border-white/10 rounded-none text-[10px] uppercase tracking-widest h-10 disabled:opacity-40">
...
export default function HomepageDesignPage() {
        ...
        try {
            ...
            if (configRes.status === "fulfilled" && configRes.value.ok) {
                ...
                setConfig((prev) => ({
                    ...prev,
                    ...data,
                    bestseller_product_ids: normalizeIdList(data?.bestseller_product_ids),
                    featured_product_ids: normalizeIdList(data?.featured_product_ids),
                    new_arrival_product_ids: normalizeIdList(data?.new_arrival_product_ids)
                }))
            } else {
                hasAnyFailure = true
            }
    ...
    const handleSave = async () => {
        ...
        try {
            const payload = {
                ...config,
                bestseller_product_ids: normalizeIdList(config.bestseller_product_ids),
                featured_product_ids: normalizeIdList(config.featured_product_ids),
                new_arrival_product_ids: normalizeIdList(config.new_arrival_product_ids)
            }
            ...
            if (res.ok) {
                showToast(data.message || "Homepage updated successfully", "info")
                await fetchData()
            } else {
                showToast(data.error || "Failed to update homepage", "info")
            }
        } catch (err) {
    ...
    const toggleProductSelection = (id: string, listKey: keyof ConfigType) => {
        setConfig(prev => {
            ...
            const newList = currentList.includes(normalizedId)
                ? currentList.filter(i => i !== normalizedId)
